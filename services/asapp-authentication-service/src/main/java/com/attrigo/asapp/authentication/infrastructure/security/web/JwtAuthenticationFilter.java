...
import static com.attrigo.asapp.authentication.infrastructure.config.SecurityConfiguration.API_WHITELIST_POST_URLS;
import static com.attrigo.asapp.authentication.infrastructure.config.SecurityConfiguration.API_WHITELIST_URLS;
import static com.attrigo.asapp.authentication.infrastructure.config.SecurityConfiguration.BOOTUI_WHITELIST_URLS;
import static com.attrigo.asapp.authentication.infrastructure.config.SecurityConfiguration.MANAGEMENT_WHITELIST_URLS;
import static com.attrigo.asapp.authentication.infrastructure.config.SecurityConfiguration.ROOT_WHITELIST_URLS;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull FilterChain filterChain)
        ...
        try {
            log.trace("[JWT_FILTER] Step 2/4: Decoding and verifying token: {}", encodedToken.value());
            ...
            log.debug("[JWT_FILTER] Authentication successful for subject={}", jwtAuthenticationToken.getName());
    ...
    private Set<RequestMatcher> buildExcludedMatchers() {
        ...
        Set<RequestMatcher> matchers = Stream.of(API_WHITELIST_URLS, ROOT_WHITELIST_URLS, MANAGEMENT_WHITELIST_URLS, BOOTUI_WHITELIST_URLS)
                                             .flatMap(Collection::stream)
                                             .map(path::matcher)
                                             .collect(Collectors.toSet());
        API_WHITELIST_POST_URLS.stream()
