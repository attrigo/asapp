...
import com.bcn.asapp.uaa.infrastructure.authentication.JwtNotFoundException;
    ...
    public final JwtAuthentication verifyAccessToken(String accessToken) {
        try {
            ...
            if (!decodedAccessToken.isAccessToken()) {
                throw new UnexpectedJwtTypeException(String.format("JWT %s is not a %s", decodedAccessToken.getJwt(), JwtType.ACCESS_TOKEN));
            }

            return jwtAuthenticationRepository.findByAccessToken(decodedAccessToken.getJwt())
                                              .orElseThrow(() -> new JwtNotFoundException(
    ...
    public final JwtAuthentication verifyRefreshToken(String refreshToken) {
        try {
            ...
            if (!decodedRefreshToken.isRefreshToken()) {
                throw new UnexpectedJwtTypeException(String.format("JWT %s is not a %s", decodedRefreshToken.getJwt(), JwtType.REFRESH_TOKEN));
            }
