...
- **The `build-logic` repository block moved and its order flipped (reverses §7's scaffold)**: §7 puts `gradlePluginPortal()` ahead of `mavenCentral()` in `build-logic/build.gradle.kts`. What shipped is `dependencyResolutionManagement { repositories { mavenCentral(); gradlePluginPortal() } }` in `build-logic/settings.gradle.kts` — centralized like the root build, and **Central first**, since the Portal proxies Central and Portal-first would route every plugin jar through it. Do not restore §7's order. The shipped file carries this as a comment, which is why `.claude/rules/gradle.md` states it no longer.
