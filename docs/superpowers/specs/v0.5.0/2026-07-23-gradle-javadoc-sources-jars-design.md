...
- **The `javadoc` encoding trio landed later, with the full-build subtask (beyond §5's block).** Commit `9aa092bc` added `encoding`, `docEncoding` and `charSet`, all `"UTF-8"`, alongside the doclint flag inside a single `(options as StandardJavadocDocletOptions).apply { }` in both archetype plugins — restoring the encodings Maven inherited from the Boot parent and never re-declared here. The cast is what makes them reachable: `docEncoding` and `charSet` exist only on `StandardJavadocDocletOptions`, not on the `MinimalJavadocOptions` the `options` getter is typed as, so `addBooleanOption` alone could not have set them. Recorded here because this spec, not the full-build one, is the javadoc source of record.
