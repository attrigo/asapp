...
This spec and its plan (`docs/superpowers/plans/2026-06-13-boot-ui.md`) were written before implementation. The core change shipped substantially as designed — boot-ui `1.4.0` managed once in `services/pom.xml` (`<bootui.version>`) and declared in the three API services, the shared `trust-container-gateway` dev-overlay line, the `bootui.enabled=OFF` test pins, each `DevToolingLockdownIT` extended with a `BootUiExposure` 401 assertion, the JMeter exposure plan, and the README/`TODO.md` documentation all landed as specified. **The design's headline promise of "zero security-code changes" (§2, §3, §7) did not hold**, and two configuration assumptions (§5 `optional` scope, §6 AUTO-only activation) were reversed during the manual-review pass. The sections above record the original design intent; **the canonical implementation is the current state of the POMs, property files, `SecurityConfiguration` / `JwtAuthenticationFilter`, the three `DevToolingLockdownIT`, and `asapp-tooling-exposure.jmx` on this branch**, not this document. Notable deltas:

- **Security code *was* changed (reverses the §2/§3/§7 "no security-code changes" non-goal).** §7 reasoned that boot-ui's own highest-precedence permit-all chain meant the app's authenticated `/**` chain never needed touching. In practice the app's `JwtAuthenticationFilter` (a `OncePerRequestFilter`, not part of boot-ui's chain) still ran on every `/bootui` request — redundantly, and logging noise on each panel poll. `fix(security)` (`aba59859`) added a `BOOTUI_WHITELIST_URLS` constant (`/bootui`, `/bootui/**`) to each `SecurityConfiguration` and wired it into `JwtAuthenticationFilter.buildExcludedMatchers()` so the filter skips the console paths, with each parameterized `shouldNotFilter` test extended to match. The lockdown reasoning still holds: these paths are excluded only from the *filter*, not added to the authorization whitelist, so under `prod` (boot-ui force-disabled, no permit-all chain) `.anyRequest().authenticated()` still rejects `/bootui` with **401** — exactly as §7/§8 predicted and as each `DevToolingLockdownIT.BootUiExposure` now asserts (`/bootui` and `/bootui/api`).

- **The no-token log branch was downgraded WARN → DEBUG (a second, related security-code change, unanticipated by the spec).** `fix(security)` (`df4dc30c`) changed `JwtAuthenticationFilter`'s missing-Bearer-token branch from `log.warn("…Authentication failed…")` to `log.debug("…continuing as anonymous request")` across the three services: boot-ui's anonymous panel polling was flooding the console with spurious "authentication failed" warnings. A request without a token is a normal anonymous request (the filter already continues the chain); genuine token failures (invalid, expired, wrong type) stay WARN.

- **`<optional>true</optional>` was dropped from the boot-ui dependency (reverses §5's "mirror the `spring-boot-devtools` block exactly").** §5 specified `runtime` scope + `optional`, copying the devtools block. But Spring Boot's repackage strips optional dependencies from the fat jar, so the starter was excluded from the built images — the console activated only on local `mvn spring-boot:run` (full classpath), never under the docker dev stack. `fix(deps)` (`00a1c378`) removed `<optional>` from all three services so the starter ships in the image; the version is still managed once via `<bootui.version>1.4.0</bootui.version>`, and devtools stays `optional` as before.

- **`bootui.enabled=ON` was added to the dev overlay (reverses §6's "No `bootui.enabled` override in main config … left to boot-ui's AUTO default").** `feat(config)` (`466476b6`) set `bootui.enabled=ON` in `central-config/application-dev.properties` so the console activates deterministically whenever `dev` is active, independent of whether devtools is on the classpath. Served only under `dev`, so `prod` stays force-disabled. The `trust-container-gateway=ON` line shipped as designed alongside it.

- **The §11 compatibility risk resolved benignly.** The primary unknown — boot-ui `1.4.0` (advertised against Spring Boot 4.1.x) on this repo's 4.0.5 — caused no trouble: the tasks-service pilot compiled, booted, and served the console, so neither fallback (pin an older release, or defer to the v0.5.0 4.1 upgrade) was needed, and the change replicated to all three services as the §11 sequence planned.

- **Incidental cleanups folded in.** Manual review also removed a duplicated `postgresql` dependency in `asapp-authentication-service` (`c58dd668`) and sorted the boot-ui POM entry and JMeter samplers to match the formatter / JMeter-GUI output (`d7a7f55e`, `922571a1`, `a0bfb3ff`).

**For future boot-ui or console-gating edits**, treat the current POMs (non-optional `runtime` boot-ui), `central-config/application-dev.properties` (`bootui.enabled=ON` + `trust-container-gateway=ON`), the per-service `BOOTUI_WHITELIST_URLS` / `JwtAuthenticationFilter` exclusion, each `DevToolingLockdownIT`'s `BootUiExposure` 401 assertions, and the `asapp-tooling-exposure.jmx` boot-ui samplers as the template; this spec is preserved as a record of the original design intent — including the "zero security-code changes" goal the implementation had to walk back.
