...
- **`asapp-stress.jmx`**: the same comprehensive read+write journey run by many concurrent threads (nested random-count loops scale the data volume), for
  observation in Grafana. The default stack is `docker,dev`, where the dev tooling is enabled which adds minor per-query overhead; for representative numbers
  run stress against a `docker,prod` stack, where the tooling is force-disabled.
