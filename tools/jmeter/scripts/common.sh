...
# Do NOT add `set -euo pipefail` here: this file is sourced, so the parent
# script's strict mode already applies, and an `exit` below aborts the run.
#

# Resolve paths relative to this file (tools/jmeter/scripts/common.sh) so the
# run scripts don't each recompute them.
...
# Runs JMeter non-GUI against <plan-file> and writes a timestamped .jtl, .log and
