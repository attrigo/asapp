...
- Tasks are grouped into a fixed, ordered set of `### <bucket>` headings; show one only when it has items:
...
- `## Backlog` holds unscheduled candidates and keeps its own shape: `### <bucket>` (same buckets as above) → `#### <scope>` headings → plain `*` bullets.
- Promote an item into a version when it's picked up.
