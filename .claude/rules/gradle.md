...
- The block is repeated in both archetype plugins **by design** — javadoc/sources spans two archetypes with no shared node (the common ancestor `asapp.java-conventions` is all 7 and would wrongly include the infra services), so it stays on the module-archetype axis rather than adding a concern plugin; this is **accepted elective duplication**, kept small and by-design for now — not forced boilerplate on par with the duplicated version-catalog accessor, whose duplication precompiled-script-plugin scoping *forces* (a convention script can't see a `val` declared in the plugin it applies), whereas a shared `asapp.javadoc-sources-conventions` plugin here is a genuine alternative; **never** put it in `asapp.java-conventions` (breaks parity and doc-lints never-linted infra source)
- Repay the duplication once a **third** module joins the {2 libs + 3 domain services} activation set, or the block grows beyond bare task registration (e.g. javadoc links, `-quiet`, offline options) — extract to a precompiled `asapp.javadoc-sources-conventions` plugin applied by both archetypes, which needs no `build-logic/build.gradle.kts` classpath entry, matching how this concern is already core-Gradle-only
