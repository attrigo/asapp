...
- Validation errors extend `ProblemDetail` with an `errors` property containing a list of `InvalidRequestParameter(location, field, message)` where `location` is a `ParameterLocation` enum (`PATH`, `QUERY`, `HEADER`, `BODY`)
